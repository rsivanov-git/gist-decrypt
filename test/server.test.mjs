...
function encrypt(plain, aad = 'surge-personal/Proxy-List.txt/v1') {
...
test('compatible envelope; empty and binary content; rejects tampering, key and AAD mismatch', () => {
  for (const plain of [Buffer.alloc(0), Buffer.from('Привет\r\n'), randomBytes(1024)]) assert.deepEqual(decrypt(encrypt(plain), key), plain);
  const encoded = encrypt(Buffer.from('private'));
  assert.throws(() => decrypt(encoded, randomBytes(32)));
  assert.throws(() => decrypt(encrypt(Buffer.from('private'), 'wrong'), key));
  for (const field of ['nonce', 'tag', 'data']) {
    const e = JSON.parse(encoded), b = Buffer.from(e[field], 'base64'); b[0] ^= 1; e[field] = b.toString('base64');
    assert.throws(() => decrypt(JSON.stringify(e), key));
  }
  assert.throws(() => decrypt('{}', key));
});
test('configuration validation never exposes input secrets', () => {
  const env = { GIST_URL: 'https://gist.githubusercontent.com/user/abcdef/raw/Proxy-List.txt', PROXY_ENCRYPTION_KEY: key.toString('base64') };
...
test('each request fetches fresh data; no cache or conditional 304; health and routing', async t => {
  ...
  for (let i = 1; i <= 2; i++) {
    const res = await fetch(url + '/Proxy-List.txt', { headers: { 'If-None-Match': '*' } });
